...
class TabFitOptions(ttk.Frame):
    def __init__(self, parent):
        ...
        self.specs_list = []
        self.specs_labelname_list = ['Name', 'Cuvettesize', 'Concentration', 'Solution']
    ...
    def refreshSpecs(self):
        ...
        self.fit_specs_vars_values_list[2].set(session_cache.concentration)
    ...
    def saveSpecsToSession(self):
        ...
        session_cache.concentration = self.fit_specs_vars_values_list[2].get()
    ...
    def fitParamManipulationWidget(self):
        ...
        for elements in self.fit_param_label_names:
            ...
            i += 1
        self.createFitManipulationPolynomElement(i)

    def createFitManipulationPolynomElement(self, i):
        label_name = ttk.Label(self.fit_param_manipulation_frame, text=f"Polynom :")
        label_name.grid(column=1, row=i * 10 + 2, padx=25, pady=10, columnspan=1, rowspan=2)  # , sticky="E")
        button_edit_minus = ttk.Button(self.fit_param_manipulation_frame, text="-", width=6,
                                 command=lambda: self.updateSessionCachePolynomDegree(-1))
        button_edit_minus.grid(column=4, row=i * 10 + 2, padx=5, pady=5, columnspan=2, rowspan=2)
        label = ttk.Label(self.fit_param_manipulation_frame, text=session_cache.fit_polynomial_degree)
        label.grid(column=6, row=i * 10 + 2, padx=5, pady=5, columnspan=2, rowspan=2)
        button_edit_plus = ttk.Button(self.fit_param_manipulation_frame, text="+", width=6,
                                       command=lambda: self.updateSessionCachePolynomDegree(1))
        button_edit_plus.grid(column=8, row=i * 10 + 2, padx=5, pady=5, columnspan=2, rowspan=2)


    def updateSessionCachePolynomDegree(self,add_sub):
        next_value = session_cache.fit_polynomial_degree + add_sub
        if next_value >= 0 and next_value <= session_cache.fit_polynomial_degree_max:
            session_cache.fit_polynomial_degree = next_value
        self.refreshFitParams()
    ...
    def createSingleFitParamManipulationElementDisplay(self,i):
        ...
        button_edit = ttk.Button(self.fit_param_manipulation_frame, text="edit", width=6, command=lambda: self.updateFitParamManipulation('edit',i))
