...
class GraphWidget(tk.Frame):
    ...
    def initTypeDependingValues(self):
        ...
        if self.type == "graph creator results":
            self.initBaselineValues()
    ...
    def plot(self):
        ...
        if self.type == "fit results":
            ...
            for element in session_cache.fitted_params_wo_polynom:
                try:
                    tmp = session_cache.referencespectra_list[i][2]
                except:
                    tmp = 0
                new_list.append([i,element[1] * tmp])
                i += 1
            self.plotData([new_list],[])
        if self.type == "graph creator results":
            self.plotData(session_cache.graph_creator_graph_list,[])
        if self.type == "jump":
        ...
        if self.type == "multi Analysis fits":
            self.plotData(current_analysis.getAllGraphData(),[])
        if self.type == "baseline":
    ...
    def calcPlot(self, data_list,line_list):
        ...
        for element in data_list:
            ...
            elif self.type == "graph creator results":
                self.ax.plot([x[0] for x in element],[x[1] for x in element], color= design_scheme.color_scheme_graph[i])
            elif self.type == "multi Analysis fits":
                self.ax.plot([x[0] for x in element],[x[1] for x in element], color= design_scheme.color_scheme_graph[i], label=current_analysis.data_list[i].name, linestyle= 'dashed')
            else:
        ...
        if self.type == "multi Analysis fits" and len(current_analysis.data_list) > 0:
